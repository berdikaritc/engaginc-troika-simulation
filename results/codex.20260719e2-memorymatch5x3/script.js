...
const state = { cards: [], scores: [0, 0], player: 0, groupsLeft: TRIO_COUNT, known: new Map() };
...
function setup() {
  ...
  const selected = shuffle([...OBJECTS]).slice(0, TRIO_COUNT);
...
function updateProgress() {
  $('#progress-fill').style.width = `${((TRIO_COUNT - state.groupsLeft) / TRIO_COUNT) * 100}%`;
...
function chooseCards() {
  ...
  const matchChance = .10 + ((TRIO_COUNT - state.groupsLeft) / (TRIO_COUNT - 1)) * .60;
